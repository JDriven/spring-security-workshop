...
Since all requests pass through `LeaveRequestService`, this seems like the perfect place to add our security restrictions.
...
Go ahead and add these annotations now.
...
Users should only be able to submit and view their own leave requests, while users with HR role have no such restrictions.
To protect these methods we will use a mix of pre- and post authorization annotations.
