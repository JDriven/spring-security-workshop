...
----
<1> We provide a `@MockBean JwtDecoder` to https://docs.spring.io/spring-security/reference/5.7.0/servlet/test/mockmvc/oauth2.html#_authentication_requestpostprocessor[prevent the deferred JWT decoder initialization] from triggering once a JWT is passed in.
<2> We mock the `JwtDecoder.decode(String)` to always return the same `Jwt` value.
<3> We provide a dummy value for the `Authentication: Bearer` header.
