...
.src/main/resources/application.yml snippet to configure oauth2 resource server.
[source,yaml]
----
spring:
  security:
...
https://docs.spring.io/spring-security/reference/5.7.2/servlet/test/mockmvc/csrf.html[`SecurityMockMvcRequestPostProcessors#csrf()`] from `spring-security-test`.
...
https://docs.spring.io/spring-security/reference/5.7.2/servlet/test/mockmvc/oauth2.html#testing-jwt[The JWT can be configured] in a number of different ways.
...
----
<1> We provide a `@MockBean JwtDecoder` to https://docs.spring.io/spring-security/reference/5.7.2/servlet/test/mockmvc/oauth2.html#_authentication_requestpostprocessor[prevent the deferred JWT decoder initialization] from triggering once a JWT is passed in. +
...
https://docs.spring.io/spring-security/reference/5.7.2/servlet/authorization/method-security.html[the Authorization chapter of Spring Security].
...
=== Granted authorities
Keycloak by default wraps the assigned roles inside a claim called `realm_access`.
Within that claim there's a `roles` field, which hold a collection of roles.
...
Because of the nested structure of these claims, we have to https://docs.spring.io/spring-security/reference/5.7.2/servlet/oauth2/resource-server/jwt.html#oauth2resourceserver-jwt-authorization-extraction[manually extract the authorities].
...
class KeycloakRealmRoleConverter implements Converter<Jwt, JwtAuthenticationToken> {
	...
	public JwtAuthenticationToken convert(Jwt jwt) {
		...
		List<String> roles = (List<String>) realmAccess.getOrDefault("roles", List.of()); <1>
